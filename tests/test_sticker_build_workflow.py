...
class StickerBuildWorkflowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_imported_catalog_is_eligible_until_the_agent_requests_exclusions(self):
        ...
        for controls, expected in cases:
            with self.subTest(controls=controls):
                ...
                self.assertEqual({runtime.get_by_sticker_id(item['sticker_id']).sticker_id for item in result.output['candidates']}, expected)
    ...
    async def test_text_only_route_uses_explicit_description_fallback_and_keeps_real_evidence(self):
        ...
        self.assertNotIn('appearance_embedding_source', _candidate_payload(matches[0]))
        self.assertEqual(matches[0].entry.asset.provenance['visual_embedding_source'], 'description')
    ...
    async def test_agent_reference_survives_revision_removal_restore_restart_and_operator_lookup(self):
        from tgchatbot.tools.sticker_send import StickerSendSelectedTool
        first_id = self.picture('first/one.png', 'red')
        second_id = self.picture('second/two.png', 'blue')
        await self.builder.build(self.root)
        first, second = await self.catalog.get_asset(first_id), await self.catalog.get_asset(second_id)
        self.assertNotEqual(first.agent_id, second.agent_id)
        self.assertEqual((await self.catalog.get_asset(first.agent_id)).asset_id, first_id)
        self.assertEqual((await self.catalog.inspect_asset(first.agent_id))['asset_id'], first_id)
        self.assertEqual(set(await self.catalog.export_corrections(asset_ids=[first.agent_id])), {first_id})
        await self.catalog.update_pack_descriptions({'first': 'Warm greetings'})
        self.assertEqual((await self.catalog.get_asset(first_id)).agent_id, first.agent_id)
        revision = await self.catalog.begin_revision(source_root=str(self.root), recipe={}, assets=[second], dimensions=3)
        await self.catalog.activate(revision)
        restarted = StickerCatalog(StickerCatalogStore(self.store), self.root)
        self.assertEqual(await restarted.aagent_sticker_id(first_id), first.agent_id)
        self.assertIsNone(await restarted.aagent_sticker_id('unknown'))
        self.assertFalse(restarted.loaded)
        missing = await StickerSendSelectedTool(restarted).run({'selected_sticker_id': first.agent_id}, ToolContext('fixture', 'Participant'))
        self.assertFalse(missing.output['ok'])
        self.assertEqual(missing.stickers, [])
        self.assertEqual((await restarted.aget_available(second.agent_id)).sticker_id, second_id)
        restored = await self.catalog.begin_revision(source_root=str(self.root), recipe={}, assets=[first, second], dimensions=3)
        await self.catalog.activate(restored)
        for reference in (first.agent_id, first_id):
            sent = await StickerSendSelectedTool(restarted).run({'selected_sticker_id': reference}, ToolContext('fixture', 'Participant'))
            self.assertEqual(sent.output['sticker_id'], first.agent_id)
            self.assertEqual(sent.stickers[0].source_id, first_id)
            self.assertEqual(sent.stickers[0].content_sha256, first.content_hash)
        third_id = self.picture('third/three.png', 'green')
        await self.builder.build(self.root)
        third = await self.catalog.get_asset(third_id)
        self.assertNotIn(third.agent_id, {first.agent_id, second.agent_id})
        self.assertEqual((await self.catalog.get_asset(first_id)).agent_id, first.agent_id)
        calls = len(self.provider.calls)
        await self.builder.build(self.root, regenerate_ids=[first.agent_id])
        self.assertEqual(len(self.provider.calls), calls + 1)
        self.assertEqual((await self.catalog.get_asset(first_id)).agent_id, first.agent_id)

    async def test_old_catalog_backfill_preserves_cards_vectors_and_stable_references(self):
        self.picture('first/one.png', 'red')
        self.picture('second/two.png', 'blue')
        await self.builder.build(self.root)
        before = await self.catalog.load_snapshot()
        async with self.store.pool.connection() as conn:
            await conn.execute('DROP TABLE sticker_catalog_references')
        await self.catalog.initialize()
        backfilled = await self.catalog.load_snapshot()
        await self.catalog.initialize()
        restarted = await StickerCatalogStore(self.store).load_snapshot()
        self.assertEqual({asset.agent_id for asset in backfilled.assets}, {asset.agent_id for asset in restarted.assets})
        for original, loaded in zip(before.assets, backfilled.assets, strict=True):
            self.assertEqual(original.asset_id, loaded.asset_id)
            self.assertEqual(original.card, loaded.card)
            self.assertEqual(original.corrections, loaded.corrections)
            self.assertEqual(original.provenance, loaded.provenance)
            self.assertTrue(np.array_equal(original.image_vector, loaded.image_vector))
            self.assertTrue(np.array_equal(original.reading_vectors, loaded.reading_vectors))
        self.assertEqual(len({asset.agent_id for asset in backfilled.assets}), 2)
