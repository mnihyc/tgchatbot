...
# Maintenance commands import offline; model calls belong only to explicit builds.
docker run --rm --network none "$image" sh -ec '
python - <<"PY"
import numpy as np
import psycopg
import runpy
np.testing.assert_allclose(np.asarray([[1, 2], [3, 4]]) @ np.asarray([[1, 2], [3, 4]]), [[7, 10], [15, 22]])
