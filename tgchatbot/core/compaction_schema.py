...
class ToolSpanMemoryBlock(_CompactionModel):
    ...
    tool_timeline: list[str] = Field(description='Chronological bullets: actual caller -> tool or action -> purpose and meaningful result. Framework tool calls belong to the agent, not a nearby participant.')
...
class EpisodeMemoryBlock(_CompactionModel):
    ...
    tool_usage: list[str] = Field(default_factory=list, description='Meaningful tool usage from raw messages or parent L0 blocks: actual caller -> purpose -> tool -> important result. Framework tool calls belong to the agent, not a nearby participant.')
    ...
    interaction_timeline: list[str] = Field(description='Chronological statements and events with their actors. Preserve later replies, changed plans, and conversational developments; not every statement needs a consequence.')
    results_or_takeaways: list[str] = Field(description='Useful answers, developments, resolutions, or social takeaways from this slice. Keep a temporary status tied to its conversational time.')
    decisions: list[str] = Field(description='Chosen path, commitment, or next-step decision that should influence later turns.')
    open_loops: list[str] = Field(description='What still needs follow-up, confirmation, or later work.')
