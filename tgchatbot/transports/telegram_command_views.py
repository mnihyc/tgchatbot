...
PARAMETERS = {
    ...
    'max_interaction_rounds': Parameter('tools', 'Interaction rounds', 'Tool interaction rounds allowed before a final request with tools disabled.'),
...
def status_view(status: dict, flow: dict, topic: str = '') -> str:
    ...
    if topic == 'tools':
        ...
        lines = [
            '🛠 <b>Tools</b>',
            f"Mode: {_code(status.get('mode'))} · interaction rounds: {_number(status.get('max_interaction_rounds'))}",
